...
namespace UnleashedApi
{
  class Program
  {
    ...
    {
      //A Guid needs to be provided when we request API to create a new customer. 
      //Therefore, if we provide a non-Guid, API will not be able to create a new customer successfully.

      //Positive flow
      var request = new HttpRequestMessage(HttpMethod.Post, $"/Customers/{newCustomer.Guid}");

      //Negative flow
      //var request = new HttpRequestMessage(HttpMethod.Post, $"/Customers/1");
